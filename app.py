...
from CraflandMap_info_pb2 import MapRequest, MapInfo  # Changed from a_pb2 import
...
def decode_hex(hex_string):
    ...
    map_info = MapInfo()  # Changed from CSGetPlayerPersonalShowRes
    map_info.ParseFromString(byte_data)
    return map_info
...
@app.route('/map-info', methods=['GET'])  # Changed endpoint name
def main():
    ...
    # Create MapRequest instead of uid_generator
    ...
    try:
        # Changed endpoint to match CraflandMapService
    ...
    try:
        map_info = decode_hex(hex_response)
    except Exception as e:
        return jsonify({"error": f"Failed to parse Protobuf: {str(e)}"}), 500

    # Create response based on MapInfo structure
    result = {
